...
from shared.schemas import IncidentRecord, IncidentStatus, Severity, AnomalyType, MetricsSnapshot, AnomalyEvent
...
def get_historical_incidents(count: int = 100) -> list[IncidentRecord]:
    ...
    for i in range(count):
        ...
        if i % 10 == 0:
            ...
            inc.human_feedback = "The issue was actually a downstream API ratelimit, not a database timeout. Requires scaling the application gateway, NOT the database."
...
async def main():
    ...
    print(f"⚙️ Running Fine-Tuning Auto-Labeler...")
