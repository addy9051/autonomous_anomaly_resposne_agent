        ...
        with tracer.start_as_current_span("reward_agent.evaluate") as span:
            ...
            try:
                response = await self.llm.ainvoke(
                    [
                        SystemMessage(content="You are a Senior Reliability Engineer (SRE) evaluating an autonomous response agent's performance."),
...
1. **logical_consistency**: Did the RCA actually follow from the provided metrics? (e.g., if latency is high due to CPU, did they blame the DB?)
